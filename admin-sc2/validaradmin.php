...
$consulta = "SELECT * FROM usuarios where usuario_mail='$usuario' and usuario_clave='$claveE'"; //$claveE , esta comparando con alguna clave = de la BD , por eso se cambio de $clave el cual tomaba el valor del input , por la claveE la cual es la encriptacion del input.
$resultado=mysql_query($consulta, $conexion);//devuelve un valor de coincidencias
	...
	if($activa == '1'){  
		...
		if($encript == $claveE){
			if($id_mail_p!=''){
                ...
                $rs_pendiente = mysql_query($sql_pendiente,$conexion);
                $row_pendiente= mysql_fetch_array($rs_pendiente);
