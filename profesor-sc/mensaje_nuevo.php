    ...
    <a style="font-size:150%; cursor: pointer;" id="cerrarM"><i class="far fa-times-circle"></i>OCULTAR</a>
                        ...
                        <label style="float: right;" for="checkall"><u>Marcar - Desmarcar todos</u></label>                        
                          ...
                          <tr>
                            ...
                            $sql_profesores = "SELECT * FROM usuarios as u, perfiles as p WHERE usuario_estado = '1' AND p.perfil_id = u.perfil_id";

                            $rs_profesores = mysqli_query($conexion, $sql_profesores);

                            $contador = '1';

                            while($row_profesores = mysqli_fetch_array($rs_profesores))

                            {

                            ?>

                             <td><?=$row_profesores['usuario_nombres']." ".$row_profesores['usuario_apellidos']." ".$row_profesores['perfil_nombre']?>

                                <?php 

                            $usuario_id = $row_profesores['usuario_id'];

                            ?>                         

                              <input type="checkbox" style="float: right !important; width: 10px;" id="checkbox" name="profesor[]" value="<?=$row_profesores['usuario_id']?>"> 

                                 <input type="hidden" name="id" value="<?=$row_profesores['usuario_id']?>"></td>
