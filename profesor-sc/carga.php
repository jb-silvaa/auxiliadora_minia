...
<div class="contenedor-100">
 ...
 $sql_limite_a = " SELECT * FROM periodos

             where periodo_activo = '1'

             ";

             $rs_limite_a = mysqli_query($conexion, $sql_limite_a);

             $row_limite_a = mysqli_fetch_Array($rs_limite_a);

             $fecha_actual_a = date('Y-m-d');
			...
			while($unidad >= $contador){ 
				...
				while($row_cursos = mysqli_fetch_array($rs_cursos)){
					...
					$mes = $array[$contador+1];

					#<td>"."MENSUAL ".$contador."</td>";
					...
					echo "<tr>
							...
							if($fecha_actual_a > $row_limite_a['periodo_limite_mensual']){
								if($row_cursos['carga_archivo'] != ''){
									echo "<td>--</td>";
								}else{
									echo "<td><a href='carga_nuevo.php?id=".$cur_asig."&unidad=".$array[$contador]."'><i class='fas fa-upload'></i></a></td>";	
								}
									echo "<td>--</td>";
							}else{
								...
								if($row_cursos['carga_archivo'] == ''){
									...
									if(is_file($row_cursos['carga_archivo'])) { 
									echo "<td><a href='".$row_cursos['carga_archivo']."'><i class='fas fa-search fa-lg'></i></a></td>";
									}
									else
									{
										echo "<td>Archivo Corrupto, favor eliminar y resubir.</td>";
									}
								}
						...
						}else if($row_cursos['carga_aprobacion'] == '0'){

							echo "<td><i class='fas fa-exclamation-circle fa-lg'></i></td>";

						}else if($row_cursos['carga_aprobacion'] == '1'){
		...
		<table class="header2">
			...
			$row_cursos = mysqli_fetch_array($rs_cursos);
				...
				echo "<tr>
					...
					if($fecha_actual_a > $row_limite_a['periodo_limite_anual']){
