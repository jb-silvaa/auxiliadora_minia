<?php
    ...
    $rs_activo = mysqli_query($conexion, $sql_activo);
    $row_activo = mysqli_fetch_array($rs_activo);
    $periodo_activo = $row_activo['periodo_periodo'];
    //asigno nombre tipo del nivel
    if($semestre == 1){ $profesor_nombre = 'PRIMER SEMESTRE'; $mes_bajo = '1'; $mes_alto = '6';} 
    if($semestre == 2){ $profesor_nombre = 'SEGUNDO SEMESTRE'; $mes_bajo = '7'; $mes_alto = '12';} 
    $sql_datos = "SELECT ca.curso_asignatura_id, 
                        evaluacion_fecha,
                        DATE_FORMAT(evaluacion_fecha,'%d-%m-%Y') AS evaluacion_fecha_esp, 
                        evaluacion_nombre, 
                        ...
                        WHERE YEAR(e.evaluacion_fecha) = '$periodo_activo'
                        AND MONTH(e.evaluacion_fecha) >= '$mes_bajo'
                        AND MONTH(e.evaluacion_fecha) <= '$mes_alto'
                        AND ca.nivel_id = '$nivel'
                        AND ca.letra_id = '$letra'
                        ...
                        ORDER BY n.nivel_nombre ASC, l.letra_nombre ASC, e.evaluacion_fecha ASC";

    $rs_datos = mysqli_query($conexion, $sql_datos);
    ...
    <br><br><br><br>
        ...
        <tr>
            <th style="width:480px;font-size:80%;text-align:center;"><b>SEMESTRE: '.$profesor_nombre.'</b></th>
        </tr>
        </table>';

    $mes_actual = '0';
    while($row_datos = mysqli_fetch_array($rs_datos))
    {
        ...
        $row_mes = mysqli_fetch_array($rs_mes);
        ...
        $html.='<table cellpadding="2" style="border: 1px solid black;" border="1">
                <tr>
                    <th style="width:50px;font-size:75%">'.$row_datos['evaluacion_fecha_esp'].'</th>
    ...
    $filename = "listado_evaluaciones_".$time.".pdf";
