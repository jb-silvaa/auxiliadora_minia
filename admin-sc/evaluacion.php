...
$eval_id = $_GET['id2'];
$sql_profe = "SELECT profesor_nombres, profesor_apellidos FROM profesores as p, cursos_asignaturas as ca WHERE ca.curso_asignatura_id = '$cur_asig' AND p.profesor_id = ca.profesor_id";
$rs_profe = mysqli_query($conexion, $sql_profe);
$row_profe = mysqli_fetch_array($rs_profe);
$nombre_profesor = $row_profe['profesor_nombres']." ".$row_profe['profesor_apellidos'];
	...
	<div id="page-content-wrapper">
            ...
            <div class="container" style="width: 90% !important;">
                ...
                <div class="row">
                    ...
                    <h1 style="font-size:20px;">Curso: <?=$curso?>, Asignatura: <?=$asignatura?>, Profesor(a): <?=$nombre_profesor?>)</h1>
					<table class="header2" style="width: 100% !important; margin: 10px 0px !important; ">
						...
						<?php 
							...
							{
								$prueba_id = $row_cursos['evaluacion_id'];
								...
								echo "<td>".$row_cursos['evaluacion_nombre']."</td>
									...
									if($row_cursos['evaluacion_archivo'] == ''){
										...
										<td><a id="myanchor<?=$row_cursos['evaluacion_id']?>" download="<?=$curso."-".$asignatura."(".$row_cursos['evaluacion_id'].")"?>" href="../profesor-sc/<?=$evaluacion_archivo?>"><i class='fas fa-search fa-lg' id='open<?=$row_cursos['evaluacion_id']?>'></i></a></td>										
										...
										<script>
										    ...
										    $('#open<?=$row_cursos['evaluacion_id']?>').on('click', function(){
										        ...
										        $("#myanchor<?=$row_cursos['evaluacion_id']?>")[0].click()
									...
									if($row_cursos['evaluacion_encargado'] == ''){
										...
										echo "<td>".$row_cursos['evaluacion_encargado']."</td>";

									}	?>
									<td><a href='listado_evaluaciones_eliminar.php?id=<?=$prueba_id?>&ca_id=<?=$cur_asig?>&vengo=1' onclick = "javascript: return confirm('Desea Eliminar Esta Evaluación?');"><i class='far fa-times-circle fa-lg'></i></a></td></tr>
						<?php	}
